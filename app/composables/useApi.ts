...
class ApiClient {
  ...
  async updateSchedulerTask(id: number, task: Partial<CreateScheduledTaskRequest>): Promise<ScheduledTask> {
    return this.updateScheduledTask(id, task)
  }

  async deleteSchedulerTask(taskId: number): Promise<void> {
    return this.deleteScheduledTask(taskId)
  }

  async executeSchedulerTask(taskId: number): Promise<{ execution_id: number }> {
    await this.triggerScheduledTask(taskId)
    // Возвращаем совместимый формат
    return { execution_id: Date.now() } // Используем timestamp как ID
  }
